/* 阅读模式「点上 1/3 往回翻、点下 2/3 往下翻」的回归测试。
 *
...
(async () => {
    ...
    const d = await page.evaluate(() => {
        ...
        return {
            ...
            top10: probe(0.10),   // 上 1/3 内 → 往回（负）
            top30: probe(0.30),   // 仍在上 1/3 内 → 往回（负）
            mid40: probe(0.40),   // 已过分界 → 往下（正）
            bottom90: probe(0.90) // 下 2/3 → 往下（正）
        };
    });
    eq('C1 分界比例是 0.33', d.ratio, 0.33);
    ok('C2 点最上面 10% → 往回翻', d.top10 < 0, '位移 ' + d.top10);
    ok('C3 点 30%（分界线之上）→ 往回翻', d.top30 < 0, '位移 ' + d.top30);
    ok('C4 点 40%（分界线之下）→ 往下翻', d.mid40 > 0, '位移 ' + d.mid40);
