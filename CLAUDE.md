...
- **窗口化渲染（进会话防卡顿）**：长篇会话进入只渲染「上次阅读位置→末尾」窗口，早章上滑到顶自动补渲染。`chatRenderAllMessages(windowed)`（**只进会话那次传 true**）、`chatComputeRenderStart`（以 `reading_pos_<convId>.idx` 为锚、前留 `READING_RENDER_BUFFER=4`）、`chatLoadEarlier`（补渲染并用高度差修 scrollTop）。**阅读定位靠「第几条消息+第几段」锚点（`.reading-merged p[data-p]`），不靠像素，所以少渲早章不影响回原位。**
  ...
  - **整章 content-visibility（2026-07-08 加，真正治卡/烫的关键）**：光按「消息条数」封窗口不够——共读**一条消息≈一整章**，20 条就 20 万字，DOM 里那几章可见正文才是压手机的根源。给整章正文（`chatBuildMsgEl` 里加 `.reading-chapter` 类，共读带批注 / 翻译对照都加）挂 CSS `content-visibility:auto; contain-intrinsic-size:auto 1200px;`——**屏外的章浏览器直接跳过排版+绘制**，页面里挂多少万字，滚动都只付「可见那一两章」的成本，不再卡、不再发热。⚠️**只加在整章级别，绝不加在段落（`.reading-merged p`）上**——段落级会让章内偏移变估算、把「记忆阅读位置」搞不准（那是 2026-07 前退回过的老坑）；`intrinsic-size` 必须带 `auto` 让浏览器记住每章真实高度，屏外收起后 scrollHeight 不跳、定位不漂。`readingRestoreScrollPos` 的逐帧校正循环本就为它写的。测试 scratchpad `test_cv_pos.js`（10 章真实结构，存位置→滚走→恢复，误差 0px、2 帧收敛）。
