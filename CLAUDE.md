...
- **翻译「前情梳理」（分集版，2026-07-16 重构）**：存 `chatMemoryTables.recap`，**数组** `[{text, srcIdx}]`（旧版是单块字符串，`chatTransRecapSegs`/`chatTransRecapEnsureArr` 读取时按需迁移成一段、`srcIdx:null` 排最前）。**每集一段**：`chatTransUpdateRecap(src, manual, srcIdx)` 只压「这一集」（≈300 字，`max_tokens:1100`），按 `srcIdx` 去重（重压/♻️重做替换不新增）、加完 `sort` 保持集号有序。发新集自动压上一集（`chatTransUpdateRecapFromPrev`，靠 user 消息上的 `_recapped` 防重复）、手动「压缩本集」（`chatTransRecapCurrent`）、**「补齐前 N 集」批量**（`chatTransRecapAll`，`chatTransRecapMissing` 收集有原文但无段的集、逐集压带进度、切会话即停）。显示复用摘要段 `_segHtml`（折叠+字数+🗑删除 `chatTransRecapDelSeg`+♻️重做 `chatTransRecapRegenSeg`）；集号 `chatTransEpisodeNo`=srcIdx 前的 user 数；追问才回喂 AI（`chatTransRecapJoin` 按集号拼，逐段翻译不带、省额度）。**⚠️坑：`chatTransHarvestMemory` 提取四表后重建 `chatMemoryTables` 必须带回 `recap` 字段**（曾漏掉→一提取记忆就把前情抹空、看着像「不积累」，2026-07-16 已修）。测试 scratchpad `test_recap_segs.js` / `test_recap_render.js` / `test_recap_backfill.js`（playwright 在旧 scratchpad `48f02160.../node_modules`，用 `NODE_PATH=` 指过去跑）。
